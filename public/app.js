...
// 快捷键符号：macOS 显示 ⌘，其他平台显示 Ctrl
const cmdKey = () => (state.platform === 'darwin' ? '⌘' : 'Ctrl');

const state = {
  cwd: null, home: null, platform: window.fanboxEnv?.platform || 'unknown', sep: '/',
...
async function enterEditMode(e) {
  ...
  const leave = async () => {
    if (getValue && getValue() !== baseline) {
      const ok = await confirmDialog(`有未保存的改动，放弃并退出？（保存请点取消后按 ${cmdKey()}S）`);
      if (!ok) return;
    }
  ...
  if (await mona.load()) {
    ...
    body.innerHTML =
      `<div class="editor-bar"><button id="ed-save" class="primary">保存</button><button id="ed-cancel" class="ghost-btn">完成</button><span class="editor-hint">${cmdKey()}S 保存 · ${cmdKey()}F 查找 · Esc 完成</span></div>` +
      `<div id="ed-host" class="mona-host"></div>`;
    ...
    body.innerHTML =
      `<div class="editor-bar"><button id="ed-save" class="primary">保存</button><button id="ed-cancel" class="ghost-btn">完成</button><span class="editor-hint">${cmdKey()}S 保存 · Esc 完成</span></div>` +
...
async function mdEditor(e, data, mode = 'rich') {
  ...
  const render = async (m) => {
    ...
    mona.disposeIfAny(); crepe.disposeIfAny();
    body.innerHTML =
      `<div class="editor-bar"><button id="md-mode" class="ghost-btn">${m === 'rich' ? '源码' : '富文本'}</button><span id="md-status" class="editor-hint">自动保存 · ${cmdKey()}S 立即保存</span></div>` +
...
function maybeShowGuide() {
  ...
  ov.innerHTML = `<div class="guide-card">
    ...
    <ul>
      <li><b>${cmdKey()}K</b> 全局搜文件和文件夹；<b>${cmdKey()}↵</b> 把项目直接在编辑器整包打开；<code>内容:关键词</code> 搜文件里的字</li>
