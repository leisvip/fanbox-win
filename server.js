...
// 仅 macOS 可用，Windows/Linux 直接返回 null 走 grep 兜底
...
async function findAgentBin(name) {
  ...
  return new Promise((resolve) => {
    if (process.platform === 'darwin' || process.platform === 'linux') {
      // macOS/Linux: 使用 zsh/bash 查找命令路径
      ...
      // Windows: 使用 where 命令查找可执行文件
      execFile('where', [name], { timeout: 8000 }, (err, stdout) => {
        const out = String(stdout || '').trim().split('\n').pop();
        // where 返回的是完整路径，如 C:\Users\...\claude.exe
        resolve(!err && out && out.includes('\\') ? out : null);
      });
    } else {
      resolve(null);
    }
