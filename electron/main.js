...
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    // macOS: hiddenInset 显示红绿灯 + vibrancy 毛玻璃；Windows/Linux: 标准标题栏
    ...(isMac ? {
...
function screenshotDir() {
  ...
  if (platform === 'darwin') {
    ...
    // Windows: 默认保存到 Pictures\Screenshots 或 OneDrive\Pictures\Screenshots
    const pictures = path.join(os.homedir(), 'Pictures');
    const screenshots = path.join(pictures, 'Screenshots');
    if (fs.existsSync(screenshots)) return screenshots;
    // 检查 OneDrive 路径
    const oneDrivePictures = path.join(os.homedir(), 'OneDrive', 'Pictures', 'Screenshots');
...
function startShotWatch() {
  ...
  try {
    shotWatcher = fs.watch(dir, { persistent: false }, (evt, filename) => {
      ...
      // macOS: Screen Shot, 截屏, CleanShot; Windows: Screenshot, 截图
...
ipcMain.handle('clip:file', (e, { path: p }) => new Promise((resolve) => {
  ...
  if (process.platform === 'darwin') {
    // macOS: 使用 AppleScript 复制文件到剪贴板（Finder 可粘贴）
    ...
    const psScript = `Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Clipboard]::SetFileDropList([System.Collections.Specialized.StringCollection]@('${p.replace(/'/g, "''")}'))`;
    execFile('powershell', ['-Command', psScript], (err) => resolve({ ok: !err, error: err && err.message }));
  } else {
    // Linux: 使用 xclip（如果可用）
    execFile('xclip', ['-selection', 'clipboard', '-t', 'text/uri-list', '-o'], { input: `file://${p}` }, (err) => resolve({ ok: !err, error: err && err.message }));
  }
}));
...
ipcMain.handle('pty:cwd', (e, { id }) => new Promise((resolve) => {
  ...
  if (process.platform === 'darwin' || process.platform === 'linux') {
    ...
    // Windows: 使用 PowerShell 获取进程可执行路径，然后取父目录
    // 注意：Windows 无法直接获取进程的 cwd，但可以获取进程路径
    exec(`powershell -Command "Get-CimInstance Win32_Process -Filter 'ProcessId=${p.pid}' | Select-Object -ExpandProperty ExecutablePath"`, (err, stdout) => {
